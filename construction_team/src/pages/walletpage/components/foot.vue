<template>
    <div class="center-box">
        ...
        <div class="money-content" v-for="(item1,index) of newlist" :key="index">
            <div class="list-item">
                <div class="item-date">{{item1.createTime | formatDate}}</div>
                <div class="item-address">{{item1.addresses.details}}</div>
            </div>
...
export default {
  ...
  computed: {
    ...
    showfoot () {
      if (this.list2.length <= 3 && this.list2.length === 1) {
        return true
      } else {
        return false
      }
